...
if len(sys.argv[1:]) > 0 :
   #tstart, nread = map(int, sys.argv[1:3])
   #nread = map(int, sys.argv[1])
   nread = int(sys.argv[1])
   ncfile =  sys.argv[2]
   print nread 
   print ncfile 
else:
   nread = 10; ncfile='sst_301.15/wrfout_d01_0001-04-11_00:00:00'
print 
print "---------------------------------------------------------------------------"
print "nread =  %5i, %s"%(nread, ncfile)
print
#nc=CDF('wrfinput_d01')
...
#-----------------------------------------------------------------------
qfx = qfx_nc.get(start=[tstart,0,0],count=[nread,ny,nx] )
qfx = qfx*86400

acqfx = acqfx_nc.get(start=[tstart,0,0],count=[2,ny,nx],stride=[nread,1,1] )
acqfx = acqfx

rain = rain_nc.get(start=[tstart,0,0],count=[2,ny,nx],stride=[nread,1,1] )
raindailymean = mean(mean(rain,2),1)

acqfxdailymean = mean(mean(acqfx,2),1)
qfxdaily = mean(mean(qfx,2),1)
#print 'qfxdaily=',mean(diff(acqfxdailymean))
print 'meanqfxdaily=',mean(qfxdaily)

tsk = tsk_nc.get(start=[tstart,0,0],count=[2,ny,nx],stride=[1,1,1] )
sst = mean(mean(mean(tsk,2),1),0)
#print acqfxdailymean 

#print acqfxdailymean
qfx_d = (acqfxdailymean[-1]-acqfxdailymean[0])/(nread)*dayfactor
rain_d = (raindailymean[-1]-raindailymean[0])/(nread)*dayfactor
if 1==1:
...
achfx = achfx_nc.get(start=[tstart,0,0],count=[2,ny,nx],stride=[nread,1,1] )
...
znfac=(znw[0,0:-1]-znu)/(-diff(znw));

#znu = nc_varget(ff,'ZNU',[0 0],[1 zsize(2)]); znu=znu(1,:);
#znw = nc_varget(ff,'ZNW',[0 0],[1 zsize(2)+1]); znw=znw(1,:);
