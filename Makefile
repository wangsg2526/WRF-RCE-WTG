...
em_rce : wrf
	@ echo '--------------------------------------'
	( cd main ; $(MAKE) MODULE_DIRS="$(ALL_MODULES)" SOLVER=em IDEAL_CASE=rce em_ideal )
	( cd test/em_rce ; /bin/rm -f wrf.exe ; ln -s ../../main/wrf.exe . )
	( cd test/em_rce ; /bin/rm -f ideal.exe ; ln -s ../../main/ideal.exe . )
	( cd test/em_rce ; /bin/rm -f README.namelist ; ln -s ../../run/README.namelist . )
	( cd test/em_rce ; /bin/rm -f gribmap.txt ; ln -s ../../run/gribmap.txt . )
	( cd test/em_rce ; /bin/rm -f grib2map.tbl ; ln -s ../../run/grib2map.tbl . )
	( cd run ; /bin/rm -f ideal.exe ; ln -s ../main/ideal.exe . )
	( cd run ; if test -f namelist.input ; then \
	/bin/cp -f namelist.input namelist.input.backup ; fi ; \
               /bin/rm -f namelist.input ; ln -s ../test/em_rce/namelist.input . )
	( cd run ; /bin/rm -f input_sounding ; ln -s ../test/em_rce/input_sounding . )
	@echo "build started:   $(START_OF_COMPILE)"
	@echo "build completed:" `date`
